...
// Client Script for Doctype: Stock Entry

frappe.ui.form.on('Stock Entry', {
  setup(frm) {
    // Show only Stock Entry Types where hidden = 0
    frm.set_query('stock_entry_type', () => ({ filters: { hidden: 0 } }));
  },

  onload(frm) {
    frm._last_stock_entry_type = frm.doc.stock_entry_type || null;
    update_visibility(frm);
  },

  refresh(frm) {
    update_visibility(frm);
    hide_specific_top_buttons(frm); // keep all; hide only "Create" / "Get Items From"
  },

  stock_entry_type(frm) {
    update_visibility(frm, { source: 'stock_entry_type' });
  },

  material_issue_purpose(frm) {
    update_visibility(frm, { source: 'material_issue_purpose' });
  },

  material_receipt_purpose(frm) {
    update_visibility(frm, { source: 'material_receipt_purpose' });
  }
});

/** ------------------ Helpers ------------------ **/

function update_visibility(frm, ctx = {}) {
  const type = frm.doc.stock_entry_type || '';
  const lastType = frm._last_stock_entry_type || null;
  const typeChanged = type !== lastType;

  // Default: hide & unrequire (don't clear here)
  hide_and_unrequire(frm, 'material_issue_purpose');
  hide_and_unrequire(frm, 'material_receipt_purpose');
  hide_and_unrequire(frm, 'other_material_issue_purpose');
  hide_and_unrequire(frm, 'other_material_receipt_purpose');

  // === Material Issue ===
  if (type === 'Material Issue') {
    show_and_require(frm, 'material_issue_purpose');

    if (typeChanged) {
      safe_clear(frm, ['material_receipt_purpose', 'other_material_receipt_purpose']);
      set_opening_flag(frm, false);
    }

    if ((frm.doc.material_issue_purpose || '') === 'Other Purpose') {
      show_and_require(frm, 'other_material_issue_purpose');
    } else {
      hide_and_unrequire(frm, 'other_material_issue_purpose');
      safe_clear(frm, ['other_material_issue_purpose']);
    }

    frm._last_stock_entry_type = type;
    return;
  }

  // === Material Receipt ===
  if (type === 'Material Receipt') {
    show_and_require(frm, 'material_receipt_purpose');

    if (typeChanged) {
      safe_clear(frm, ['material_issue_purpose', 'other_material_issue_purpose']);
    }

    if ((frm.doc.material_receipt_purpose || '') === 'Other Purpose') {
      show_and_require(frm, 'other_material_receipt_purpose');
    } else {
      hide_and_unrequire(frm, 'other_material_receipt_purpose');
      safe_clear(frm, ['other_material_receipt_purpose']);
    }

    // Opening flag ONLY when purpose exactly matches "Opening Stock Balance"
    const isOpening = (frm.doc.material_receipt_purpose || '') === 'Opening Stock Balance';
    set_opening_flag(frm, isOpening);
  ...
  if (typeChanged) {
    safe_clear(frm, [
      'material_issue_purpose',
      'other_material_issue_purpose',
      'material_receipt_purpose',
      'other_material_receipt_purpose'
    ]);
  }
...
function show_and_require(frm, fieldname) {
  frm.toggle_display(fieldname, true);
  frm.set_df_property(fieldname, 'reqd', 1);
}

function hide_and_unrequire(frm, fieldname) {
  frm.toggle_display(fieldname, false);
  frm.set_df_property(fieldname, 'reqd', 0);
}

// Clear only when we know the field is no longer applicable
function safe_clear(frm, fields) {
  (fields || []).forEach((f) => {
    if (frm.doc[f]) frm.set_value(f, null);
  });
}

// Set is_opening = "Yes"/"No" safely
function set_opening_flag(frm, yes) {
  const desired = yes ? 'Yes' : 'No';
  if (frm.doc.is_opening !== desired) {
    frm.set_value('is_opening', desired);
  }
}

/**
 * Hide ONLY top buttons/menus whose visible label/title is exactly:
 *   - "Create" (or "إنشاء")
 *   - "Get Items From" (or "جلب العناصر من")
 * Keep everything else (including custom buttons).
 * Robust against toolbar re-renders & translations.
 */
function hide_specific_top_buttons(frm) {
  const HIDE_LABELS = [
    'Get Items From',
    'Get Items From | جلب الاصناف من',
    'Create',
    'Create | إنشاء',
    'جلب العناصر من',
    'إنشاء'
  ...
  const matches = (txt) => {
    ...
    return HIDE_LABELS.some(l => t === String(l).trim().toLowerCase());
  };

  // 1) Try the API remove (inner buttons) – with and without __()
  try {
    HIDE_LABELS.forEach((label) => {
      try { frm.page.remove_inner_button(__(label)); } catch (e) {}
      try { frm.page.remove_inner_button(label); } catch (e) {}
    });
  } catch (e) { /* no-op */ }

  // 2) DOM fallback: hide buttons & dropdown items
  const hideByDom = () => {
    try {
      ...
      $page.find('.page-actions .btn, .standard-actions .btn, .btn-group .btn').each(function () {
        const $btn  = $(this);
        const label = ($btn.text() || '').trim();
        const title = ($btn.attr('title') || '').trim();
        const aria  = ($btn.attr('aria-label') || '').trim();

        if (matches(label) || matches(title) || matches(aria)) {
          const $group = $btn.closest('.btn-group, .actions-btn-group');
          if ($group.length) $group.hide();
          $btn.hide();
        }
      });

      // Dropdown menu items (Actions / Create groups)
      $page.find('.dropdown-menu .dropdown-item, .actions-btn-group .dropdown-item').each(function () {
        const $it = $(this);
        const txt = ($it.text() || '').trim();
        if (matches(txt)) {
          const $group = $it.closest('.btn-group, .actions-btn-group');
          $it.hide();
          if ($group.length) {
            const groupLabel = ($group.find('> .btn, > .dropdown-toggle').first().text() || '').trim();
            if (matches(groupLabel)) $group.hide();
          }
        }
      });

      // Some themes put label only on the group toggle
      $page.find('.btn-group').each(function () {
        const $group = $(this);
        const toggleText = ($group.find('> .btn, > .dropdown-toggle').first().text() || '').trim();
        if (matches(toggleText)) $group.hide();
      });
    } catch (e) { /* no-op */ }
  };

  // 3) Re-apply after toolbar rebuilds
  const enableObserver = () => {
    try {
      const wrapper = frm.page && frm.page.wrapper && frm.page.wrapper.get(0);
      if (!wrapper) return;

      if (frm._toolbarObserver) {
        try { frm._toolbarObserver.disconnect(); } catch (e) {}
      }

      const observer = new MutationObserver(() => hideByDom());
      observer.observe(wrapper, { childList: true, subtree: true });
      frm._toolbarObserver = observer;
    } catch (e) { /* no-op */ }
  };

  hideByDom();
  if (frappe.after_ajax) frappe.after_ajax(hideByDom);
  setTimeout(hideByDom, 50);
  setTimeout(hideByDom, 200);
  enableObserver();
}
